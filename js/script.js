...
async function searchBooks(keyword, experience) {
  ...
  async function searchBooks(keyword, experience) {
    ...
    try {
      ...
      const bookList = document.createElement('ul');
      const existingTitles = []; // 重複チェックのための配列
      ...
      if (data && data.items) {
        data.items.forEach(item => {
          ...
          const imageLinks = volumeInfo.imageLinks;
          const thumbnail = imageLinks ? imageLinks.thumbnail : 'noimage.png';
          const amazonLink = volumeInfo.previewLink; // Amazonページへのリンク
  
          // タイトルの重複チェック
          if (!existingTitles.includes(title)) {
            existingTitles.push(title);
            const bookItem = createBookItem(title, authors, thumbnail, amazonLink);
            bookList.appendChild(bookItem);
          }
        });
      }
